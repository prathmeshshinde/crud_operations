...
const AddEngi: React.FC = () => {
  const [addEngineer, setAddEngineer] = useState<InterfaceEngi>({
    firstName: "",
    lastName: "",
    emailId: "",
  });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setAddEngineer({ ...addEngineer, [e.target.name]: e.target.value });
  };

  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    EngineerService.createEngineer(addEngineer)
      .then((res) => {
        if (res && res.data) {
          navigate("/adminengi");
        }
  ...
  return (
    <div>
      ...
      <div className="container mt-3">
        <div className="row">
          <div className="col">
            <p className="h3 text-success">Add Engineer</p>
            <p>
